...
class ImageLoader:
    def __init__(self, disk):
        ...
        self.disk_folder_name = 'temp1'

    def download_current_state(self):
        is_end = -1
        while self.run or is_end > 0:
            time.sleep(50)
            self.is_downloading_current_state = True
            time.sleep(5)

            self.disk.download(self.disk_folder_name, f'{self.disk_folder_name}_0.zip')
            ...
            files = os.listdir(f'{self.disk_folder_name}_0{os.sep}{self.disk_folder_name}')

            for file in files:
                shutil.move(f'{self.disk_folder_name}_0{os.sep}{self.disk_folder_name}{os.sep}{file}',
    ...
    def transform_images(self):
        while self.run or len(self.image_to_transform_queue) > 0:
